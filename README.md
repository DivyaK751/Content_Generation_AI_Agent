...
| **Business document submitted** | [Google Doc](https://docs.google.com/document/d/1-XlgHT3fy1UvcKrlbwmLJuoH1HbjG_29t63-bfIv4ig/edit?tab=t.0) |
...
|---|---|---|
| **Supervisor** | `backend/agents/supervisor.py` | The entry point for every campaign. Reads the user's message and decides what kind of request it is — trend-based post, occasion-based post, or a clarifying question. Passes control to the right agent once intent is clear. |
| &nbsp;&nbsp;└ *Supervisor Clarify* | | Fires when intent is unclear, off-topic, or needs a follow-up. Asks one question, then loops back to the supervisor to re-classify. |
| &nbsp;&nbsp;└ *Occasion Gather* | | Collects event details (occasion type, date, tone) when the user wants occasion-based content. Interrupts to ask, then routes forward once the brief is complete. |
| &nbsp;&nbsp;└ *Channels Clarify* | | Asks the user whether to post on Instagram, send email, or both — only fires when channels are still unknown. |
...
flowchart TD
    ...
    img_agent & cap_agent & email_agent --> approval
